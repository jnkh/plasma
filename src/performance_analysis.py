...
# dat = load(results_dir + results_files[3])
# #conf=dat['conf']
...
# length = 1 + len(truth[0]) - len(pred[0])
...
#             pred,truth,disruptive_curr,length,T_max_warn,T_min_warn)
...
# P_thresh_opt = 0.01
# summarize_shot_prediction_stats(P_thresh_opt,pred,truth,disruptive_curr,T_min_warn,T_max_warn,verbose=True)




# alarms,disr_alarms,nondisr_alarms = gather_first_alarms(P_thresh_opt,pred,disruptive_curr,T_min_warn,T_max_warn)
# hist_alarms(disr_alarms,T_min_warn,T_max_warn,'disruptive alarms')
...
def get_metrics_vs_p_thresh(P_thresh_range,ttd_prime_by_shot,ttd_by_shot,disr,length, \
                           T_max_warn = 1000,T_min_warn = 30,verbose=False):
